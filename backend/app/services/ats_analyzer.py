"""
Hybrid ATS Analyzer — 3 layers (quality vs effort balance for CareerForge)

Layer 1 — Rule-based (55 pts max)
  Keywords, section completeness, quantified achievements, action verbs,
  length/structure penalties.

Layer 2 — Semantic (25 pts max)
  Chroma vector similarity between resume chunks and JD (tenant-scoped).

Layer 3 — LLM qualitative (20 pts max, optional)
  Coaching feedback only; small bounded score so it cannot dominate or
  hallucinate a full 0–100 ATS number.

Final score = L1 + L2 + L3  (0–100), fully explainable in `layers`.
"""
...
# ─── Layer weights (points out of 100) ───────────────────────────────────────
W_KEYWORDS = 30
W_SECTIONS = 12
W_PROJECTS = 3
W_QUANTIFIED = 10
W_ACTION_VERBS = 10
W_LENGTH = 8
# Layer 1 total = 73 → rescale to 55 for clean L1 cap
...
def extract_jd_keywords(job_description: str, limit: int = 45) -> list[str]:
    ...
    for tok, _ in counts.most_common(35):
        if tok not in found and tok not in known_lower:
            if any(c in tok for c in "+#./") or len(tok) >= 4:
                found.append(tok)
...
def _layer1_quantified(resume_text: str) -> dict[str, Any]:
    nums = re.findall(r"\b\d+(?:\.\d+)?%?\b|\$\d[\d,]*", resume_text or "")
    count = len(nums)
    if count == 0:
        score = 0.0
    elif count <= 2:
        score = W_QUANTIFIED * 0.45
    elif count <= 5:
        score = W_QUANTIFIED * 0.75
    else:
        score = float(W_QUANTIFIED)
    return {
        "score": round(score, 1),
        "max": W_QUANTIFIED,
...
def _layer1_action_verbs(resume_text: str) -> dict[str, Any]:
    ...
    if n >= 8:
        ...
        score = W_ACTION_VERBS * 0.8
    elif n >= 3:
        score = W_ACTION_VERBS * 0.55
    elif n >= 1:
        score = W_ACTION_VERBS * 0.3
    else:
        score = 0.0
    return {
        "score": round(score, 1),
...
def _layer1_length_structure(resume_text: str) -> dict[str, Any]:
    ...
    if 250 <= words <= 900:
        ...
        length_score, band = W_LENGTH * 0.7, "acceptable"
    elif 80 <= words < 150 or 1200 < words <= 1600:
        length_score, band = W_LENGTH * 0.4, "weak"
    else:
        length_score, band = W_LENGTH * 0.15, "poor"

    # Light bonus already baked into band; structure note
    structure_note = "good bullet usage" if bullets >= 4 else "few bullet points detected"
    if bullets < 2 and words > 150:
...
def layer1_rules(resume_text: str, job_description: str) -> dict[str, Any]:
    """Layer 1: rule-based analysis (raw then scaled to L1_CAP)."""
    ...
    summary = [
        ...
        f"Length: {length['band']} ({length['word_count']} words, {length['bullet_lines']} bullets).",
    ]
    if sec["missing"]:
